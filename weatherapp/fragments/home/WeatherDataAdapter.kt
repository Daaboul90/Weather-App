...
import com.example.weatherapp.databinding.ItemContainerForcastBinding

class WeatherDataAdapter(
    private val onLocationClicked: () -> Unit):RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when(viewType){
            ...
            INDEX_FORECAST -> ForecastViewHolder(
                ItemContainerForcastBinding.inflate(
                    LayoutInflater.from(parent.context),
                    parent,
                    false
                )
    ...
    inner class CurrentWeatherViewHolder(
        ...
        fun bind(currentWeather: CurrentWeather){
            with(binding){
                ...
                // Ensure correct data types for String.format() and handle nullability
                textTemperature.text = String.format("%s\u00B0C", currentWeather.temperature ?: "N/A")
                textWind.text = String.format("%s km/h", currentWeather.wind ?: "N/A")
                textHumidity.text = String.format("%s%%", currentWeather.humidity ?: "N/A")
                textRainy.text = String.format("%s%%", currentWeather.chanceOfRain ?: "N/A")
            }
        }
    }
    ...
    inner class ForecastViewHolder(
        private val binding: ItemContainerForcastBinding
